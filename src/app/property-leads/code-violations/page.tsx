...
import { fetchCodeViolations, CodeViolation } from '@/services/socrata'
...
export default function CodeViolationsPage() {
  ...
  const [results, setResults] = useState<CodeViolation[]>([])
  ...
  const handleSearch = async () => {
    ...
    try {
      const data = await fetchCodeViolations({
  ...
  const handleDownload = () => {
    ...
    link.setAttribute('download', `code_violations_${selectedCity}_${new Date().toISOString().split('T')[0]}.csv`)
    ...
    <div className="p-6">
      <div className="mb-6">
        ...
        <p className="text-gray-600 mt-1">
          Find properties with code violations - potential distressed properties and motivated sellers
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            ...
            >
              ...
              {Object.entries(getCitiesGroupedByState('codeViolations')).map(([state, cities]) => (
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        {isLoading ? (
                    ...
                    <td className="px-4 py-4">
                      <span className={`px-2 py-1 text-xs font-medium rounded-full ${
                        violation.status?.toLowerCase().includes('open') || violation.status?.toLowerCase().includes('active')
                          ...
                          : violation.status?.toLowerCase().includes('closed') || violation.status?.toLowerCase().includes('complete')
          ...
          <div className="text-center py-12 text-gray-500">
            Select a city and date range, then click Search to find code violations.
          </div>
        )}
      </div>
    </div>
